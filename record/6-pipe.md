...
nestjs 에서 HTTP 요청 값에 대한 유효성 검증 및 데이터 변환의 역할을 하는 pipe 에 대한 내용이다.

## 본론

### pipe?

`pipe` 는 HTTP 요청에 대한 데이터 변환과 유효성 검증 역할을 한다.
- 변환 : 입력 데이터를 원하는 형태로 변환합니다(예: 문자열을 정수로).
- 유효성 검사 : 입력 데이터를 평가하고, 유효한 경우 변경 없이 그대로 전달하고, 그렇지 않으면 예외를 발생시킨다.

```mermaid
flowchart TD
  A[Pipe </br> 유효성 검사 및 변환] --> B{검사 통과?}
  B --> |yes| C[Controller]
  B --> |no| D[pipe 정의 </br> Exception]
  C --> D[Service]
  D --> E[Repository]
```
